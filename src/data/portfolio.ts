...
export const about = {
  narrative:
    "3년간 운영하던 피자집이 코로나19로 문을 닫은 후, 디지털 포렌식에서 새로운 길을 찾았습니다. 문제의 원인을 하나씩 좁혀가는 분석 과정에 깊이 몰입할 수 있다는 확신으로 보안 분야에 뛰어들었고, 공격을 직접 설계하고 그 흔적을 추적하며 \"어떻게 뚫렸는가\"를 넘어 \"왜 이 시점에 이 조직이 타깃이 되었는가\"까지 질문하는 분석가로 성장하고 있습니다.",
  valueProposition:
    "오류의 원인을 체계적으로 좁혀가는 분석 습관을 바탕으로, 침해사고 대응의 최전선에서 아티팩트가 말하는 이야기를 읽어내는 CERT/DFIR 분석가가 되고자 합니다.",
  keyStrengths: [
    {
      label: "공격자 관점의 위협 분석",
      detail:
        "취약한 환경을 직접 구축하고 공격 시나리오를 설계·실행한 뒤, 3중 로그 시스템으로 흔적을 추적하며 공격-방어 양면의 분석력 체득",
    },
    {
